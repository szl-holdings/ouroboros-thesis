...
[![Runtime tests](https://img.shields.io/badge/runtime%20tests-1%2C372%2F1%2C372-2da44e?style=flat-square)](https://github.com/szl-holdings/ouroboros)
...
v3 introduces the **Lutar Invariant Λ**, a closed-form scalar in [0, 1] that aggregates nine independent runtime-trust axes — Cleanliness, Horizon, Resonance, Frustum, Geometry, Invariance, Moral, Being, Soul — into a single auditable number. The invariant is derived from first principles, instrumented in a production runtime (`@szl-holdings/ouroboros` v6.1.0), and validated against **1,372 tests** (925 TypeScript, 447 Python). Where v1 was a position paper and v2 was empirical, v3 is the formal scalar law.
...
The v1 paper was a position and systems-design preprint. v2 is the empirical companion: a shipped reference implementation (then 142/142 tests, now 1,372/1,372 in v3), three production case studies on the same kernel (A11oy, Sentra, Amaru), closure of the v1 §9 experimental agenda, and a §3.7 falsification ledger.
...
The runtime that implements the v3 invariant is published at [`szl-holdings/ouroboros`](https://github.com/szl-holdings/ouroboros) — `@szl-holdings/ouroboros` v6.1.0, **1,372/1,372 tests passing** (925 TypeScript + 447 Python). The seven domain products that consume the runtime — A11oy, Sentra, Amaru, Counsel, Terra, Vessels, Carlota Jo — are showcased under the [`szl-holdings`](https://github.com/szl-holdings) organization.
